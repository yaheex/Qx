...
[rewrite_local]
^https:\/\/bd-api\.kuwo\.cn\/api\/(ucenter\/users|play\/listening\/user|service\/(home\/index|banner\/myPage|advert\/watch)) url script-response-body https://raw.githubusercontent.com/yaheex/Qx/refs/heads/main/chxm1023/bodian.js
...
if(vipa.test($request.url)){
  yahe1023.data.payInfo = {
      "expireDate" : 4092599349000,
      "isSigned" : 1,
      "lastPayType" : 1,
      "isVip" : 1
    };
  yahe1023.data.payRights = {
      "headPendant" : {
        "id" : 11,
        "name" : "音波",
...
if(ad.test($request.url)){
  yahe1023.data.bannerList = [];
  if (yahe1023.data.moduleList && yahe1023.data.moduleList.length > 0) {
   yahe1023.data.moduleList = yahe1023.data.moduleList.filter(item => item.name !== "轮播图" && item.name !== "波点实验室");
  };
}
