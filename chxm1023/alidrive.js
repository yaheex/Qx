...
[rewrite_local]
^https?:\/\/(api|member)\.(aliyundrive|alipan)\.com\/(.+\/(users|activity|user\/get)|((business|apps)\/.+\/users|adrive\/.+\/user)) url script-response-body https://raw.githubusercontent.com/yaheex/Qx/main/yahee/alidrive.js
...
if (url.indexOf(vipb) != -1){
 body = body.replace(/\{.+\}/g, '\{  "description" : "有效期至2099-09-09",  "titleNotice" : "YaHee,",  "activityAction" : "smartdrive://webview?url=https%3A%2F%2Fpages.aliyundrive.com%2Fmobile-page%2Fweb%2Fmembership.html%3FdisableNav%3DYES%26renew%3DYES",  "rightButtonText" : "SVIP",  "activityText" : "解锁部分功能，不完全解锁",  "identity" : "svip",  "backgroundImage" : "https://gw.alicdn.com/imgextra/i4/O1CN01cwAW0u1GPG2oa6WW7_!!6000000000614-2-tps-654-212.png",  "titleImage" : "https://gw.alicdn.com/imgextra/i2/O1CN01snE6rA1pVg95HyRBl_!!6000000005366-2-tps-214-49.png",  "backgroudImage" : null  }');
}
...
if (url.indexOf(adrive) != -1){
  yahee.capacity_level_info = {
    "capacity_type" : "normal"
  };
  yahee.user_capacity_limit_details = {
...
if ($request.url.indexOf(hengtiao) != -1){
  delete yahee.result;  //横条信息
  body = JSON.stringify(yahee);
}

if ($request.url.indexOf(guolv) != -1){
  yahee.coreFeatures.items = yahee.coreFeatures.items.filter(item => item.code !== "AI_ASSISTANT");  //图搜
  delete yahee.signIn;  //登录
  delete yahee.banners; //横幅
  delete yahee.minorBackup;  //设备列表
  delete yahee.mainBackup;  //备份还原
  body = JSON.stringify(yahee);
}
