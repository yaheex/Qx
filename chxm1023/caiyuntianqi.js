...
let yahe1024 = {}, yahe1023 = JSON.parse(typeof $response != "undefined" && $response.body || null);
...
if (typeof $response == "undefined") {
  ...
  yahe1024.headers = { ...headers, 'device-token': token };
  if (headers['app-version'] > '7.19.0') { yahe1024.headers['authorization'] = `Bearer ${token}`; }
} else {
  ...
  if (/user\?app_name/.test(url)) {
    yahe1023.result = { ...yahe1023.result, "is_vip": true, "vip_expired_at": 4092599349, "svip_given": 1, "is_xy_vip": true, "xy_svip_expire": 4092599349, "wt": { ...yahe1023.result.wt, "vip": { ...yahe1023.result.wt.vip, "expired_at": 0, "enabled": true, "svip_apple_expired_at": 4092599349, "is_auto_renewal": true, "svip_expired_at": 4092599349 }, "svip_given": 1 }, "vip_take_effect": 1, "xy_vip_expire": 4092599349, "svip_expired_at": 4092599349, "svip_take_effect": 1, "vip_type": "s" };
  }
  //新版数据
  if (/user_detail/.test(url)) {
    yahe1023.vip_info.show_upcoming_renewal = false;
    yahe1023.vip_info.svip = data;
  }
  //VIP信息
...
if (/activity\?app_name/.test(url)) {
  yahe1024.body = headers['app-version'] < '7.20.0'  ? '{"status":"ok","activities":[{"items":[{}]}]}'  : '{"status":"ok","activities":[]}';
}
